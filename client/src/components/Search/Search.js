...
const SearchModule = ({ query, setQuery, locations, setIsLoading }) => {
    ...
    const getWeatherData = (latitude, longitude) => {
        ...
        getForecast(latitude, longitude)
            ...
            .catch((err) => {
                handleError('Unable to fetch Forecast data.')
            })
    ...
    return (
        <div className="search flex justify-center max-w-5xl mb-4">
            ...
            <div className="relative w-full">
                ...
                {locations && <button type="button" className="search-btn absolute top-2/4 -translate-x-2 -translate-y-2/4 right-0 p-2 text-white bg-blue-400 rounded-full ease-in-out duration-200 active:scale-100 hover:scale-105" onClick={(e) => handleSubmit(e.target.dataset.lat, e.target.dataset.lon)}><Search /></button>}
                    ...
                    {locations.length !== 0 && 
                        <ul className='search-list p-2 bg-white rounded-md shadow-md'>
                            ...
                            {locations.map(location => {
                                return (
                                    <li key={location.latitude + location.longitude + new Date()} className='active:scale-95 hover:bg-slate-200 border-b'>
